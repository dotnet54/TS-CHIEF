...
public class TSF implements Transform{
	...
	protected int minIntervalLength = AppConfig.tsf_min_interval;
	...
	public void fit(MTSDataset train) throws Exception {
...
//	        intervals[i][0]=AppConfig.getRand().nextInt(0, datasetLength - intervalLength);	//start pos
//	        intervals[i][1]=intervals[i][0]+intervalLength;	//end pos
	...
	public MTSDataset transform(MTSDataset train) {
		int datsetSize = train.size();
		
		MTSDataset output = new MTSDataset(datsetSize, 3 * numIntervals);
...
//                System.out.println(j + " interval: " + intervals[i][j][0] + "-" + intervals[i][j][1]);
            ...
            UTimeSeries ts = new UTimeSeries(newseries, train.get_class(i));
	...
	public UTimeSeries transformSeries(UTimeSeries series) {
...
//                System.out.println(j + " interval: " + intervals[i][j][0] + "-" + intervals[i][j][1]);
        ...
        UTimeSeries ts = new UTimeSeries(newData, series.getLabel());
...
//                stDev=Math.sqrt(stDev);
	...
	public static void main(String[] args) {
		try {
			...
			MTSDataset transformed =  transformer.transform(train);
