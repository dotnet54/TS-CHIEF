...
public class BossSplitterPerNode implements NodeSplitter {
	...
	public BossSplitterPerNode(TSChiefNode node) {
...
//		}
		...
		train_hist_using_forest_transform(trainData);

	}
	
	@SuppressWarnings("unchecked")
	private void train_hist_using_forest_transform(Dataset data) throws Exception {
...
//				this.node.tree.getForest().getTransforms().get("boss")).getHistograms();
		...
		TIntObjectMap<Dataset> data_per_class = data.split_by_class(); // TODO can be done once at node
		...
		TIntObjectHashMap<UTimeSeries> examples = new TIntObjectHashMap<UTimeSeries>();
		
		
		for (int key : data_per_class.keys()) {
			int r = AppConfig.getRand().nextInt(data_per_class.get(key).size());
			UTimeSeries example = data_per_class.get(key).get_series(r);
		...
		for (boolean normMean : NORMALIZATION) {
			for (int i = 0; i < windows.length; i++) {
				for (int f = minF; f <= maxF; f += 2) {
					...
					TIntObjectMap<MTSDataset> splits = new TIntObjectHashMap<MTSDataset>();
					...
					for (int j = 0; j < data.size(); j++) {
						...
						for (int key : examples.keys()) {
							...
							UTimeSeries series = data.get_series(j);
	...
	protected int[][] createWords(final UTimeSeries[] samples, SFA sfa, int wordlen, int alphabet) {
	...
	protected int[][] createWords(final UTimeSeries[] samples, SFA sfa, int window, int wordlen, int alphabet,
...
//	    	sfa.fitWindowing(samples, window, wordlen, alphabet, norm, true);
	...
	public static BagOfPattern createBagOfPattern(final int[] words, final UTimeSeries sample, final int wordLength, int symbols) {
...
//	}
	...
	public TIntObjectMap<MTSDataset> split(MTSDataset data, int[] indices) throws Exception {
		TIntObjectMap<MTSDataset> data_per_class = data.split_by_class(); // TODO can be done once at node
		TIntObjectMap<MTSDataset> splits = new TIntObjectHashMap<MTSDataset>();
...
//	        }			
	...
	private void train_one_ensemble(MTSDataset data) throws Exception {
		...
		MTSDataset sample = Sampler.uniform_sample(data, sample_size);

		UTimeSeries[] sample_array = sample._get_internal_list().toArray(new UTimeSeries[] {});
	...
	private void train_one_ensemble_per_class(MTSDataset data) throws Exception {
		TIntObjectMap<MTSDataset> data_per_class = data.split_by_class(); // TODO can be done once at node
		...
		for (int key : data_per_class.keys()) {
			...
			UTimeSeries[] sample_array = samples_per_class.get(key)._get_internal_list().toArray(new UTimeSeries[] {});
