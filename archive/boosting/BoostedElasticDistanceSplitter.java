...
public class BoostedElasticDistanceSplitter implements NodeSplitter{
	...
	// for boosting
	protected double best_threshold=Double.POSITIVE_INFINITY;

	public BoostedElasticDistanceSplitter(TSChiefNode node) throws Exception {
		this.node = node;
	...
	public TIntObjectMap<Dataset> fit(Dataset trainData, Indexer trainIndices) throws Exception {
		...
		distanceMeasure.select_random_params(data, AppConfig.getRand());
		exemplars = new TIntObjectHashMap<double[]>(data.getNumClasses());
//		splits = new TIntObjectHashMap<Dataset>(data.get_num_classes());
		...
		for (int key : data_per_class.keys()) {
			...
			exemplars.put(branch, data_per_class.get(key).getSeries(r).data());
			branch++;
		}
	...
	public void train_binary(Dataset data) throws Exception {
...
//		splits = new TIntObjectHashMap<Dataset>(data.get_num_classes());
		...
		if (data.getNumClasses()<=nb_splits) {
			for (int i=0; i<data.getNumClasses(); i++) {
				l[i] = key_val[i];
			}
		}
		else {
			double[] weights = new double[data.getNumClasses()];
			for (int i=0; i<data.getNumClasses(); i++) {
...
//				}
	...
	public int predict(MTimeSeries query, int queryIndex) throws Exception{
	...
	@Override
	public TIntObjectMap<TIntArrayList> train_indices(Dataset data, int[] indices) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public TIntObjectMap<TIntArrayList> split_indices(Dataset data, int[] indices) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}
}
