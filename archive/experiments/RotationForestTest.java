...
public class RotationForestTest {
	...
	public static void main(String[] args) throws Exception {
        ...
        Instances train2 = WekaDataConverter.convertListDatasetToWekaInstances(train_csv);
...
//		split(train, model);
		split_using_branch(train, model);
	...
	public static TIntObjectMap<ArrayList<Instance>> split_using_branch(Instances data, Classifier model) throws Exception {

		TIntObjectMap<ArrayList<Instance>> splits = new TIntObjectHashMap<ArrayList<Instance>>(2);	//two branches
		splits.put(0, new ArrayList<Instance>());
		splits.put(1, new ArrayList<Instance>());

		RotationForestStump rotf = (RotationForestStump) model;
		
		
		if (AppConfig.verbosity > 1) {
			System.out.println("attrib: " + rotf.getAttribIndex() + "sp : " + rotf.getSplitPoint() + " dist : "
		...
		for (int i = 0; i < data.size(); i++) {
			int branch = rotf.getBranch(data.get(i));
			if (AppConfig.verbosity > 1) {
				System.out.println("pred: " + branch + " " +  rotf.getDistribution());
			}

			

	        splits.get(branch).add(data.get(i));
		}		
