...
public class MSM extends LPMultivarSimMeasure {
...
//		double[][] costMatrix = MemoryManager.getInstance().getDoubleMatrix(0);
		...
		int i, j;
		// Initialization
		costMatrix[0][0] = squaredDistancePointToPoint(series1, 0, series2, 0);
		for (i = 1; i < length1; i++) {
		...
		for (i = 1; i < length1; i++) {
			for (j = 1; j < length2; j++) {
				double diagonal, left, up;
				diagonal = costMatrix[i - 1][j - 1] + squaredDistancePointToPoint(series1, i, series2, j);
