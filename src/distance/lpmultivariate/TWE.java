...
public class TWE extends LPMultivarSimMeasure {
...
//		double[] Dj1 = MemoryManager.getInstance().getDoubleArray(1);
		...
		for (j = 1; j <= c; j++) {
			...
			if (j > 1) {
				// CHANGE AJB 8/1/16: Only use power of
				// 2 for speed
				distj1 += squaredDistancePointToPoint(series2, j-2, series2, j-1);
//				distj1 += (series2[j - 2] - series2[j - 1]) * (series2[j - 2] - series2[j - 1]);
				...
				distj1 += squaredPoint(series2, j-1);
//				distj1 += series2[j - 1] * series2[j - 1];
		...
		for (i = 1; i <= r; i++) {
			disti1 = 0;
			if (i > 1) {
				disti1 += squaredDistancePointToPoint(series1, i-2, series1, i-1);
//				disti1 += (series1[i - 2] - series1[i - 1]) * (series1[i - 2] - series1[i - 1]);
			} // OLD
			// disti1+=Math.pow(Math.abs(series1[i-2][k]-series1[i-1][k]),degree);
			else {
				disti1 += squaredPoint(series1, i-1);
//				disti1 += (series1[i - 1]) * (series1[i - 1]);
			...
			for (j = 1; j <= c; j++) {
				dist = 0;
				dist += squaredDistancePointToPoint(series1, i-1, series2, j-1);
//				dist += (series1[i - 1] - series2[j - 1]) * (series1[i - 1] - series2[j - 1]);
				// dist+=Math.pow(Math.abs(series1[i-1][k]-series2[j-1][k]),degree);
				if (i > 1 && j > 1) {
					dist +=  squaredDistancePointToPoint(series1, i-2, series2, j-2);
