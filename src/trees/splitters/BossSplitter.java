...
import dev.BOSSEnsembleClassifier;
import dev.BOSSEnsembleClassifier.BOSSModel;
...
//		}
		...
		if (AppContext.boss_transform_level ==  TransformLevel.Forest) {
			if (AppContext.boss_split_method == SplitMethod.Binary_Gini) {
				train_using_forest_transform_ginisplit(data);
			}else {
				train_using_forest_transform(data, indices);
			}
		}else {
			train_using_tree_transform(data);
		}
	...
	private void train_using_forest_transform_ginisplit(TSDataset data) {
		// TODO Auto-generated method stub
	...
	private void train_using_tree_transform(TSDataset data)throws Exception {
		BossDataset transformed_dataset = this.node.tree.boss_train_dataset_tree_level;
		...
		for (int key : boss_data_per_class.keys()) {
			int r = AppContext.getRand().nextInt(boss_data_per_class.get(key).size());
...
//		System.out.println("wgini:" + wgini + "  " + splits);
	...
	private void train_using_forest_transform(TSDataset data, int[] indices) throws Exception {
